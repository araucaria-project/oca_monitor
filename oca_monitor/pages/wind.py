...
class WindDataWidget(QWidget):
    ...
    async def reader_loop(self):
        ...
        await run_reader(
            clb=self.reader_clb_2, subject=self.weather_subject,
    ...
    def reader_clb_2(self, data, meta):
        ts = dt_ensure_datetime(data['ts']).astimezone()
        hour = ts.hour + ts.minute / 60 + ts.second / 3600
        measurement = data['measurements']
    ...
    async def reader_loop_2(self):
        await run_reader(subject=self.weather_subject, deliver_policy='last', clb=self.reader_clb_2)
        # msg = Messenger()
        #
        # # We want the data from the midnight of yesterday
        #
